...
<!-- Inspect the datasets.   -->
```{r echo=echoChunks, results='show', message=FALSE}
metaData %>% 
  dplyr::filter(name_new %in% paste0("itf_",1:7)) %>% 
  dplyr::select(name_new, label, label_graph)

ds %>% 
  dplyr::select(dplyr::contains("itf_")) %>% 
  sapply(summary)
```

```{r function-correlation-itf, echo=echoChunks, results='show', message=FALSE}
```
 

```{r data-phase-itf, echo=T, results='show', message=FALSE}
```

```{r scree-plot-itf, echo=T, results='show', message=FALSE}
```

```{r PA-psych-itf, echo=T, results='show', message=FALSE}
```
